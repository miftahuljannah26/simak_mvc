...
<body>
    <div class="app-layout">
        ...
        <main class="main-content-col">
            <h1 class="fw-bold mb-4" style="font-family:'Outfit';">Manajemen Aktivitas Mahasiswa</h1>
            
            <div class="card-custom">
                <h5 class="fw-bold mb-3" style="color:#d12264;">+ Tambah Agenda Baru</h5>
                <form action="<?= base_url('aktivitas/simpan'); ?>" method="POST">
                    <div class="row g-2">
                        <div class="col-md-4"><input type="text" name="nama_aktivitas" class="form-control" placeholder="Nama Agenda" required></div>
                        <div class="col-md-2"><select name="kategori" class="form-select"><option value="Kuliah">Kuliah</option><option value="Organisasi">Organisasi</option><option value="Pribadi">Pribadi</option></select></div>
                        <div class="col-md-3"><input type="date" name="tanggal" class="form-control" required></div>
                        <div class="col-md-2"><select name="status" class="form-select"><option value="belum">Belum</option><option value="tertunda">Tertunda</option><option value="selesai">Selesai</option></select></div>
            ...
            <div class="card-custom">
                ...
                <table class="table-custom">
                    <thead><tr><th>Nama Agenda</th><th>Kategori</th><th>Tanggal</th><th>Status</th><th>Aksi</th></tr></thead>
                    <tbody>
                        <?php if(!empty($aktivitas)): foreach($aktivitas as $row): ?>
                            <tr class="data-row">
                                <td class="fw-bold"><?= $row['nama_aktivitas']; ?></td>
                                <td><?= $row['kategori']; ?></td>
                                <td><?= date('d M Y', strtotime($row['tanggal'])); ?></td>
                                <td><span class="badge bg-secondary"><?= ucfirst($row['status']); ?></span></td>
                                <td><a href="<?= base_url('aktivitas/hapus/'.$row['id_aktivitas']); ?>" class="btn btn-sm btn-danger" onclick="return confirm('Hapus?')">Hapus</a></td>
                            </tr>
                        <?php endforeach; else: ?>
